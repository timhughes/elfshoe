...
ipxe/
├── src/
│   ├── ipxe_menu_gen.py              # Main entry point (~300 lines)
...
### 3. Main Program (`ipxe_menu_gen.py`)

**Purpose**: Orchestration and CLI interface.
...
# Custom config
python3 src/ipxe_menu_gen.py -c custom.yaml
```
...
1. **Parallel URL Validation**: Use asyncio or threading
2. **Metadata Caching**: Cache API responses locally
3. **Auto-Discovery**: Scan `distributions/` for plugins automatically
4. **Configuration Validation**: JSON Schema for config.yaml
5. **More Providers**: CentOS, Debian, Ubuntu, Windows, BSD, etc.
6. **Multi-Architecture**: ARM support
7. **CI/CD Integration**: GitHub Actions examples
8. **Web UI**: Optional web interface for configuration
...
- All code in `ipxe_menu_gen.py`
- `FedoraMetadataFetcher` hardcoded
- String check for 'fedoraproject.org'

**After** (modular):
- Core: ~150 lines across 3 files
- Distributions: ~100 lines across 3 files
- Main: ~200 lines
